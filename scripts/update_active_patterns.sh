#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
POSTMORTEM_DIR="$ROOT_DIR/queue/postmortems"
SUMMARY_DIR="$POSTMORTEM_DIR/summaries"
OUTPUT_FILE="$POSTMORTEM_DIR/ACTIVE_PATTERNS.md"

mkdir -p "$POSTMORTEM_DIR" "$SUMMARY_DIR"

extract_scalar() {
  local key="$1"
  local file="$2"
  grep -m1 "^[[:space:]]*${key}:" "$file" | sed -E "s/^[[:space:]]*${key}:[[:space:]]*//; s/^\"(.*)\"$/\1/"
}

extract_first_list_item_after_key() {
  local key="$1"
  local file="$2"
  local line
  line="$(grep -n "^[[:space:]]*${key}:" "$file" | head -1 | cut -d: -f1 || true)"
  if [ -z "$line" ]; then
    echo ""
    return
  fi
  sed -n "$((line+1)),$((line+15))p" "$file" | grep -m1 '^[[:space:]]*-[[:space:]]*"' | sed -E 's/^[[:space:]]*-[[:space:]]*"(.*)"/\1/'
}

extract_why1() {
  local file="$1"
  grep -m1 '^[[:space:]]*-[[:space:]]*why1:' "$file" | sed -E 's/^[[:space:]]*-[[:space:]]*why1:[[:space:]]*"(.*)"/\1/'
}

build_active_list() {
  : > /tmp/postmortem_active_list.txt
  local f status ts
  for f in "$POSTMORTEM_DIR"/*.yaml; do
    [ -f "$f" ] || continue
    status="$(extract_scalar "status" "$f" || true)"
    if [ "$status" = "active" ]; then
      ts="$(extract_scalar "timestamp" "$f" || true)"
      echo "${ts}|${f}" >> /tmp/postmortem_active_list.txt
    fi
  done
  sort /tmp/postmortem_active_list.txt > /tmp/postmortem_active_sorted.txt
}

render_markdown() {
  local now total f sev cat title cmd lesson why1 check
  now="$(date '+%Y-%m-%dT%H:%M:%S')"
  total="$(wc -l < /tmp/postmortem_active_sorted.txt | tr -d ' ')"

  : > /tmp/postmortem_high.md
  : > /tmp/postmortem_medium.md

  while IFS='|' read -r _ f; do
    [ -f "$f" ] || continue
    sev="$(extract_scalar "severity" "$f" || true)"
    cat="$(extract_scalar "category" "$f" || true)"
    title="$(extract_scalar "title" "$f" || true)"
    cmd="$(extract_scalar "cmd_id" "$f" || true)"
    lesson="$(extract_first_list_item_after_key "lessons" "$f" || true)"
    why1="$(extract_why1 "$f" || true)"
    check="$(extract_first_list_item_after_key "systemic_improvements" "$f" || true)"

    if [ "$sev" = "critical" ] || [ "$sev" = "high" ]; then
      {
        echo "### [${cat}] ${title}"
        echo "- **関連cmd**: ${cmd}"
        echo "- **確認観点**: ${lesson}"
        echo "- **NGパターン**: ${why1}"
        echo ""
      } >> /tmp/postmortem_high.md
    elif [ "$sev" = "medium" ]; then
      {
        echo "### [${cat}] ${title}"
        echo "- **関連cmd**: ${cmd}"
        echo "- **確認観点**: ${lesson}"
        echo "- **チェック**: ${check}"
        echo ""
      } >> /tmp/postmortem_medium.md
    fi
  done < /tmp/postmortem_active_sorted.txt

  {
    echo "# ACTIVE_PATTERNS.md — QC照合チェックリスト"
    echo ""
    echo "**最終更新**: ${now}"
    echo "**有効パターン件数**: ${total}件"
    echo ""
    echo "## ⚠️ 高優先度パターン（high/critical）"
    echo ""
    if [ -s /tmp/postmortem_high.md ]; then
      cat /tmp/postmortem_high.md
    else
      echo "（該当なし）"
      echo ""
    fi
    echo "## ⚡ 中優先度パターン（medium）"
    echo ""
    if [ -s /tmp/postmortem_medium.md ]; then
      cat /tmp/postmortem_medium.md
    else
      echo "（該当なし）"
      echo ""
    fi
  } > "$OUTPUT_FILE"
}

move_old_low_or_medium() {
  local candidate="" f sev
  while IFS='|' read -r _ f; do
    [ -f "$f" ] || continue
    sev="$(extract_scalar "severity" "$f" || true)"
    if [ "$sev" = "low" ] || [ "$sev" = "medium" ]; then
      candidate="$f"
      break
    fi
  done < /tmp/postmortem_active_sorted.txt

  if [ -n "$candidate" ]; then
    mv "$candidate" "$SUMMARY_DIR/"
    echo "moved_to_summaries: $(basename "$candidate")"
    return 0
  fi

  return 1
}

build_active_list
render_markdown
line_count="$(wc -l < "$OUTPUT_FILE" | tr -d ' ')"

while [ "$line_count" -gt 100 ]; do
  if ! move_old_low_or_medium >/tmp/update_active_patterns_move.log; then
    break
  fi
  build_active_list
  render_markdown
  line_count="$(wc -l < "$OUTPUT_FILE" | tr -d ' ')"
done

echo "updated: $OUTPUT_FILE"
echo "line_count: $line_count"
